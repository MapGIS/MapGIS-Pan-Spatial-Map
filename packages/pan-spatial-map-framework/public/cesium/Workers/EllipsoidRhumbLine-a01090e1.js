...
define(['exports', './Cartesian2-cdd21d41', './Check-c47ad5ed', './when-695b7bde', './Math-2649012a'], (function (exports, Cartesian2, Check, when, Math$1) { 'use strict';
    ...
    function calculateArcLength(ellipsoidRhumbLine, major, minor, firstLongitude, firstLatitude, secondLongitude, secondLatitude) {
        ...
        if (Math$1.CesiumMath.equalsEpsilon(Math.abs(heading), Math$1.CesiumMath.PI_OVER_TWO, Math$1.CesiumMath.EPSILON8)) {
            ...
            if (major === minor) {
                ...
                distance = (major * Math.cos(firstLatitude) * Math$1.CesiumMath.negativePiToPi(deltaLongitude)) / Math.sqrt(1 - ellipsoidRhumbLine._ellipticitySquared * sinPhi * sinPhi);
            }
        } else {
    ...
    var scratchCart1 = new Cartesian2.Cartesian3();
    var scratchCart2 = new Cartesian2.Cartesian3();

    function computeProperties(ellipsoidRhumbLine, start, end, ellipsoid) {
        var firstCartesian = Cartesian2.Cartesian3.normalize(ellipsoid.cartographicToCartesian(start, scratchCart2), scratchCart1);
        var lastCartesian = Cartesian2.Cartesian3.normalize(ellipsoid.cartographicToCartesian(end, scratchCart2), scratchCart2);

        //>>includeStart('debug', pragmas.debug);
        Check.Check.typeOf.number.greaterThanOrEquals('value', Math.abs(Math.abs(Cartesian2.Cartesian3.angleBetween(firstCartesian, lastCartesian)) - Math.PI), 0.0125);
        //>>includeEnd('debug');
        ...
        ellipsoidRhumbLine._start = Cartesian2.Cartographic.clone(start, ellipsoidRhumbLine._start);
        ellipsoidRhumbLine._start.height = 0;

        ellipsoidRhumbLine._end = Cartesian2.Cartographic.clone(end, ellipsoidRhumbLine._end);
    ...
    function interpolateUsingSurfaceDistance(start, heading, distance, major, ellipticity, result) {
        if (distance === 0.0) {
            return Cartesian2.Cartographic.clone(start, result);
        }
        ...
        return new Cartesian2.Cartographic(longitude, latitude, 0);
    }
    ...
    EllipsoidRhumbLine.prototype.findIntersectionWithLongitude = function (intersectionLongitude, result) {
        ...
        if (!when.defined(result)) {
            result = new Cartesian2.Cartographic();
        }
        ...
        if (Math.abs(Math$1.CesiumMath.PI_OVER_TWO - absHeading) <= Math$1.CesiumMath.EPSILON8) {
            ...
            result.height = 0;
            return result;
        } else if (Math$1.CesiumMath.equalsEpsilon(Math.abs(Math$1.CesiumMath.PI_OVER_TWO - absHeading), Math$1.CesiumMath.PI_OVER_TWO, Math$1.CesiumMath.EPSILON8)) {
        ...
        var leftComponent = Math.tan(0.5 * (Math$1.CesiumMath.PI_OVER_TWO + phi1)) * Math.exp((intersectionLongitude - start.longitude) / Math.tan(heading));
    ...
    EllipsoidRhumbLine.prototype.findIntersectionWithLatitude = function (intersectionLatitude, result) {
        ...
        var longitude = Math$1.CesiumMath.negativePiToPi(start.longitude + deltaLongitude);
        ...
        return new Cartesian2.Cartographic(longitude, intersectionLatitude, 0);
    };

    exports.EllipsoidRhumbLine = EllipsoidRhumbLine;

}));
//# sourceMappingURL=EllipsoidRhumbLine-a01090e1.js.map
