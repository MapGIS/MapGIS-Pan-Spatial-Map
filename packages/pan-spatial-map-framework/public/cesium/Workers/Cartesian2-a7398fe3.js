...
define(['exports', './Check-f996273c', './when-ae2e0b60', './Math-ea295399'], (function (exports, Check, when, Math$1) { 'use strict';
    ...
    /**
     * Computes the Cartesian's magnitude (length).<br/>
     * 对magnitudeSquared计算的结果进行开根号计算
     *
    ...
    var wgs84CenterToleranceSquared = Math$1.CesiumMath.EPSILON1;
    ...
    Cartographic.fromCartesian = function (cartesian, ellipsoid, result) {
        ...
        var height = Math$1.CesiumMath.sign(Cartesian3.dot(h, cartesian)) * Cartesian3.magnitude(h);
    ...
    Cartographic.equals = function (left, right) {
        return left === right || (when.defined(left) && when.defined(right) && left.longitude === right.longitude && left.latitude === right.latitude && left.height === right.height);
    };
     ...
     * @constant
     */
    Ellipsoid.MOON = Object.freeze(new Ellipsoid(Math$1.CesiumMath.LUNAR_RADIUS, Math$1.CesiumMath.LUNAR_RADIUS, Math$1.CesiumMath.LUNAR_RADIUS));
    ...
    Ellipsoid.prototype.geodeticSurfaceNormal = function (cartesian, result) {
        if (Cartesian3.equalsEpsilon(cartesian, Cartesian3.ZERO, Math$1.CesiumMath.EPSILON14)) {
            return undefined;
        }
        if (!when.defined(result)) {
            result = new Cartesian3();
    ...
    Ellipsoid.prototype.getSurfaceNormalIntersectionWithZAxis = function (position, buffer, result) {
        ...
        if (!Math$1.CesiumMath.equalsEpsilon(this._radii.x, this._radii.y, Math$1.CesiumMath.EPSILON15)) {
    ...
    Rectangle.fromDegrees = function (west, south, east, north, result) {
        west = Math$1.CesiumMath.toRadians(when.defaultValue(west, 0.0));
        south = Math$1.CesiumMath.toRadians(when.defaultValue(south, 0.0));
        east = Math$1.CesiumMath.toRadians(when.defaultValue(east, 0.0));
        north = Math$1.CesiumMath.toRadians(when.defaultValue(north, 0.0));
    ...
    Rectangle.fromCartographicArray = function (cartographics, result) {
        ...
        for (var i = 0, len = cartographics.length; i < len; i++) {
            ...
            var lonAdjusted = position.longitude >= 0 ? position.longitude : position.longitude + Math$1.CesiumMath.TWO_PI;
    ...
    Rectangle.equals = function (left, right) {
        return left === right || (when.defined(left) && when.defined(right) && left.west === right.west && left.south === right.south && left.east === right.east && left.north === right.north);
    };
    ...
    Rectangle.validate = function (rectangle) {
        ...
        Check.Check.typeOf.number.greaterThanOrEquals('north', north, -Math$1.CesiumMath.PI_OVER_TWO);
        Check.Check.typeOf.number.lessThanOrEquals('north', north, Math$1.CesiumMath.PI_OVER_TWO);

        var south = rectangle.south;
        Check.Check.typeOf.number.greaterThanOrEquals('south', south, -Math$1.CesiumMath.PI_OVER_TWO);
    ...
    Rectangle.center = function (rectangle, result) {
        ...
        var longitude = Math$1.CesiumMath.negativePiToPi((west + east) * 0.5);
    ...
    Rectangle.intersection = function (rectangle, otherRectangle, result) {
        ...
        var west = Math$1.CesiumMath.negativePiToPi(Math.max(rectangleWest, otherRectangleWest));
        var east = Math$1.CesiumMath.negativePiToPi(Math.min(rectangleEast, otherRectangleEast));
    ...
    Rectangle.union = function (rectangle, otherRectangle, result) {
        ...
        var west = Math$1.CesiumMath.convertLongitudeRange(Math.min(rectangleWest, otherRectangleWest));
        var east = Math$1.CesiumMath.convertLongitudeRange(Math.max(rectangleEast, otherRectangleEast));
    ...
    Rectangle.contains = function (rectangle, cartographic) {
        ...
        return (
            (longitude > west || Math$1.CesiumMath.equalsEpsilon(longitude, west, Math$1.CesiumMath.EPSILON14)) &&
            (longitude < east || Math$1.CesiumMath.equalsEpsilon(longitude, east, Math$1.CesiumMath.EPSILON14)) &&
     ...
     * @constant
     */
    Rectangle.MAX_VALUE = Object.freeze(new Rectangle(-Math.PI, -Math$1.CesiumMath.PI_OVER_TWO, Math.PI, Math$1.CesiumMath.PI_OVER_TWO));
    ...
    Cartesian2.angleBetween = function (left, right) {
        ...
        return Math$1.CesiumMath.acosClamped(Cartesian2.dot(angleBetweenScratch, angleBetweenScratch2));
    };
...
//# sourceMappingURL=Cartesian2-a7398fe3.js.map
