...
define(['exports', './Cartesian2-cdd21d41', './when-695b7bde', './Check-c47ad5ed', './Transforms-b822f480', './Math-2649012a'], (function (exports, Cartesian2, when, Check, Transforms, Math$1) { 'use strict';
    ...
    function addWithCancellationCheck$1(left, right, tolerance) {
        ...
        if (Math$1.CesiumMath.sign(left) !== Math$1.CesiumMath.sign(right) && Math.abs(difference / Math.max(Math.abs(left), Math.abs(right))) < tolerance) {
            return 0.0;
        }
    ...
    QuadraticRealPolynomial.computeRealRoots = function (a, b, c) {
        ...
        if (a === 0.0) {
            ...
            if (cMagnitude < aMagnitude && cMagnitude / aMagnitude < Math$1.CesiumMath.EPSILON14) {
                // c ~= 0.0.
                // 2nd order monomial: a * x^2 = 0.
                return [0.0, 0.0];
            } else if (cMagnitude > aMagnitude && aMagnitude / cMagnitude < Math$1.CesiumMath.EPSILON14) {
                // a ~= 0.0.
                // Constant function: c = 0.
                return [];
            }
        ...
        var b2 = b * b;
        var four_ac = 4.0 * a * c;
        var radicand = addWithCancellationCheck$1(b2, -four_ac, Math$1.CesiumMath.EPSILON14);
        ...
        var q = -0.5 * addWithCancellationCheck$1(b, Math$1.CesiumMath.sign(b) * Math.sqrt(radicand), Math$1.CesiumMath.EPSILON14);
        if (b > 0.0) {
            return [q / a, c / q];
        }
    ...
    function quadraticVectorExpression(A, b, c, x, w) {
        ...
        var l1 = w * (x * addWithCancellationCheck(A[Transforms.Matrix3.COLUMN1ROW0], A[Transforms.Matrix3.COLUMN0ROW1], Math$1.CesiumMath.EPSILON15) + b.y);
        var l0 = A[Transforms.Matrix3.COLUMN0ROW0] * xSquared + A[Transforms.Matrix3.COLUMN2ROW2] * wSquared + x * b.x + c;

        var r1 = wSquared * addWithCancellationCheck(A[Transforms.Matrix3.COLUMN2ROW1], A[Transforms.Matrix3.COLUMN1ROW2], Math$1.CesiumMath.EPSILON15);
        var r0 = w * (x * addWithCancellationCheck(A[Transforms.Matrix3.COLUMN2ROW0], A[Transforms.Matrix3.COLUMN0ROW2]) + b.z);
        ...
        for (var i = 0; i < length; ++i) {
            ...
            if (Math$1.CesiumMath.sign(l2) === Math$1.CesiumMath.sign(l0)) {
                left = addWithCancellationCheck(l2 * cosineSquared + l0, l1 * cosine, Math$1.CesiumMath.EPSILON12);
            ...
            var right = addWithCancellationCheck(r1 * cosine, r0, Math$1.CesiumMath.EPSILON15);
    ...
    IntersectionTests.grazingAltitudeLocation = function (ray, ellipsoid) {
        ...
        var secondAxis = Cartesian2.Cartesian3.normalize(Cartesian2.Cartesian3.cross(reference, firstAxis, secondAxisScratch), secondAxisScratch);
        var thirdAxis = Cartesian2.Cartesian3.normalize(Cartesian2.Cartesian3.cross(firstAxis, secondAxis, thirdAxisScratch), thirdAxisScratch);
        var B = bScratch;
        B[0] = firstAxis.x;
        B[1] = firstAxis.y;
        B[2] = firstAxis.z;
        ...
        var temp = Transforms.Matrix3.multiply(Transforms.Matrix3.multiply(B_T, D, tempMatrix), C, tempMatrix);
        ...
        var solutions = quadraticVectorExpression(A, Cartesian2.Cartesian3.negate(b, firstAxisScratch), 0.0, 0.0, 1.0);
        ...
        if (length > 0) {
            ...
            for (var i = 0; i < length; ++i) {
                s = Transforms.Matrix3.multiplyByVector(D_I, Transforms.Matrix3.multiplyByVector(B, solutions[i], sScratch), sScratch);
                var v = Cartesian2.Cartesian3.normalize(Cartesian2.Cartesian3.subtract(s, position, referenceScratch), referenceScratch);
            ...
            altitude = Cartesian2.Cartesian3.magnitude(Cartesian2.Cartesian3.subtract(closest, position, referenceScratch)) * Math.sqrt(1.0 - maximumValue * maximumValue);
    ...
    IntersectionTests.lineSegmentPlane = function (endPoint0, endPoint1, plane, result) {
        ...
        var difference = Cartesian2.Cartesian3.subtract(endPoint1, endPoint0, lineSegmentPlaneDifference);
        var normal = plane.normal;
...
//# sourceMappingURL=IntersectionTests-492833eb.js.map
