  ...
  <div class="search-area" ref="root">
    ...
    <div class="search-item" :key="index" v-for="(col, index) in searchCols">
      ...
      <div v-else :class="['title', { active: col.search.value }]">
        <a-popover
          @visibleChange="onVisibleChange(col, index)"
          v-model="col.search.visible"
          placement="bottom"
          :trigger="['click']"
          :get-popup-container="() => $refs.root"
        >
...
export default {
  ...
  methods: {
    ...
    momentEqual(target, source, format) {
      ...
      if (target && source && target.format(format) === source.format(format)) {
        return true
      }
      return false
    },
