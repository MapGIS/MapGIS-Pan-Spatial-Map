...
export class GeometryExp {
  public static calculateBound(dots: any[]) {
    const bound: Bound = {
      xmin: dots[0][0],
      ymin: dots[0][1],
      xmax: dots[0][0],
   ...
   * @returns {number[]}
   */
  public static getCenterOfGravityPoint(lnglats) {
    let area = 0.0 // 多边形面积
    let Gx = 0.0
    let Gy = 0.0 // 重心的x、y
  ...
  public static creatRectangleByViewRectangle(rect) {
  ...
  public static creatRectByMinMax(
    xMin: number,
    yMin: number,
    xMax: number,
    yMax: number
