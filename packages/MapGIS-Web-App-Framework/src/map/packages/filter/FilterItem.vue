<template>
  <a-row type="flex" align="middle" class="mp-filter-item">
    <a-col flex="1 1 0%">
...
<script>
import { LayerType, Feature } from '../../../model'
...
export default {
  ...
  data() {
    return {
      fields: [], // { label: string, value: string, [key: string]: unknown }[]
      firstFieldIndex: -1, // quasar 切换到antd，由于选中的值不能为对象类型，这里需要将选中下标返回，去获取选中的对象
      operIndex: defaultOper.value, // quasar 切换到antd，由于选中的值不能为对象类型，这里需要将value在相应的数组里面找出对象
      secondField: '', // 输入的值
      values: [],
      secondFieldBetweent: ['', ''], // 数值介于之间的值
      valueTypeIndex: defaultValueType.value,
      // 值类型
      valueTypeOptions: [
        { label: '值', value: 'value' },
    ...
    async getValue() {
      ...
      if (
        ...
        const values = Array.from(
          new Set(result.value.map(value => ({ value })))
        )
