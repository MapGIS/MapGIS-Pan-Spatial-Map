<template>
  <div class="mp-attribute-statistics">
...
import { UUID, LayerType, Feature, Catalog, Metadata } from '../../../model'

export default {
  name: 'MpAttributeStatistics',
  props: {
    queryParams: {
      type: Object,
      required: true
  ...
  data() {
    return {
      ...
      numberArr: ['BYTE', 'SHORT', 'INT', 'LONG', 'FLOAT', 'DOUBLE', 'BINARY']
    }
  },
  computed: {
    groupField() {
  ...
  methods: {
    ...
    async getLayerInfosByServerName() {
    ...
    // 获取图层字段信息
    async getFieldInfosByLayer(layerIndex) {
      const {
        serverType,
        serverName,
        serverUrl,
    ...
    async getResult() {
      ...
      try {
        ...
        if (
          ...
          const {
            count: totalCount
          } = await Feature.ArcGISFeatureQuery.getTotal({
            f: 'json',
            serverUrl,
