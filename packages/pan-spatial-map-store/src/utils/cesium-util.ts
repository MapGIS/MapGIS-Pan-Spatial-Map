import { Rectangle, Cartesian3 } from '@mapgis/webclient-es6-service/common'
...
class CesiumUtil {
  ...
  private webGlobeInitValue: any = undefined
  ...
  setCesiumGlobe(cesium: any, webGlobe: any) {
    ...
    if (!this.webGlobe) {
      this.webGlobe = webGlobe
      this.webGlobeInitValue = webGlobe
    } else if (
      this.webGlobe &&
      webGlobe.elementID.id !== this.webGlobe.elementID.id
    ) {
      this.webGlobe = webGlobe
    }
   ...
   */
  resetWebGlobe() {
    if (this.webGlobeInitValue) {
      this.webGlobe = this.webGlobeInitValue
    }
  }
   ...
   * @param globe
   */
  getCurrentExtent(globe = this.webGlobe) {
    // 范围对象
    const extent = {
      xmin: -180,
      ymax: 90,
