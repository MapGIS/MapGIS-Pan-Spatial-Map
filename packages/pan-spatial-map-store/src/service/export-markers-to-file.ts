...
class ExportMarkersToFile {
  getSetByMarkers(markers: Record<string, any>[]) {
    // 创建点、线、区要素数据集
    const featureSet1 = new Zondy.Object.FeatureSet()
    const featureSet2 = new Zondy.Object.FeatureSet()
    const featureSet3 = new Zondy.Object.FeatureSet()
    // 设置属性结构
    ...
    for (let i = 0; i < markers.length; i += 1) {
      ...
      const { features, title, description, coordinates } = marker
  ...
  geojsonFeatureToIgsFeature(
    ...
    if (type === 'Point') {
      ...
      fGeom = new Zondy.Object.FeatureGeometry({
        PntGeom: [new Zondy.Object.GPoint(coordinates[0], coordinates[1])]
      })
      // 描述点要素的符号参数信息
      ...
      // 设置当前点要素的图形参数信息
      GraphicInfo = new Zondy.Object.WebGraphicsInfo({
        InfoType: 1,
        PntInfo: pointInfo
      })
      fType = 1
      ...
      for (let l = 0; l < coordinates.length; l += 1) {
        ...
        coords.push(
          new Zondy.Object.Point2D(coordinates[l][0], coordinates[l][1])
        )
      }
      // 构成折线的弧段
      const gArc = new Zondy.Object.Arc(coords)
      const gAnyLine = new Zondy.Object.AnyLine([gArc])
      // 设置线要素的几何信息
      const gline = new Zondy.Object.GLine(gAnyLine)
      // 设置要素的几何信息
      fGeom = new Zondy.Object.FeatureGeometry({ LinGeom: [gline] })
      // 设置添加线要素的图形参数信息
      const clineInfo = new Zondy.Object.CLineInfo({
        Color: pntColor,
        LinStyleID: 0,
        LinStyleID2: 1,
        LinWidth: 2,
      ...
      for (let p = 0; p < coordinates.length; p += 1) {
        ...
        for (let a = 0; a < arcPoints.length; a += 1) {
          ...
          coords.push(
            new Zondy.Object.Point2D(arcPoints[a][0], arcPoints[a][1])
          )
        }
        const gArc = new Zondy.Object.Arc(coords)
        gArcs.push(gArc)
      ...
      // 构成区要素折线
      const gAnyLine = new Zondy.Object.AnyLine(gArcs)
      // 构成区要素
      const gRegion = new Zondy.Object.GRegion([gAnyLine])
      // 构成区要素的几何信息
      fGeom = new Zondy.Object.FeatureGeometry({ RegGeom: [gRegion] })
      // 设置区要素的图形参数信息
      const cRegionInfo = new Zondy.Object.CRegionInfo({
        EndColor: 1,
        FillColor: pntColor,
        FillMode: 0,
        OutPenWidth: 1,
