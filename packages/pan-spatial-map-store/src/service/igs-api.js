...
import '../libs/webclient-service-framework'
import '../libs/webclientExtent-datastore-service'
...
export function igsQueryFeature(option) {
  ...
  if (option.gdbp) {
    ...
    queryParam.proj =
      option.srsIds || systemConfigInstance.config.projectionName
  } else {
    // 矢量文档
...
export function dataStorePgQueryFeature(gdbp, option) {
  ...
  queryParam.sref = option.sref || systemConfigInstance.config.projection

  let domain = option.domain || null
  const ip = option.ip || systemConfigInstance.config.DataStoreIp
  const port = option.port || systemConfigInstance.config.DataStorePort
  if (!domain) {
    const protocol = option.protocol || defaultServer.protocol
    domain = `${protocol}://${ip}:${port}`
  }
