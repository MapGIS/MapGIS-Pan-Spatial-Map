...
import '../libs/webclientExtent-datastore-service.js'
...
export interface FeatureQueryParam {
  ...
  geometry?: Record<string, any> // 几何类型的图形参数, 包含geometryType几何类型（例如：point，circle，rect，line，polygon等）
...
class QueryFeatures {
   ...
   */
  public query(option: FeatureQueryParam, combine?: boolean) {
    if (!option) {
      return null
    }
    this.systemConfig = systemConfigInstance.config
    if (option.isDataStoreQuery) {
      // datastore
      return this.dataStoreQuery(option)
    } // igs
    return this.igsQuery(option, combine)
  }
   ...
   */
  public async igsQuery(option: FeatureQueryParam, combine?: boolean) {
  ...
  public igsQueryFeature(option: FeatureQueryParam) {
    ...
    if (option.gdbp) {
      // 矢量图层
      queryParam = new Zondy.Service.QueryByLayerParameter(option.gdbp)
      queryParam.proj = option.srsIds || this.systemConfig.projectionName
    } else {
      // 矢量文档
      queryParam = new Zondy.Service.QueryParameter()
    }
    ...
    queryParam.rule = new Zondy.Service.QueryFeatureRule({
      CompareRectOnly:
    ...
    queryParam.struct = new Zondy.Service.QueryFeatureStruct({
      IncludeAttribute:
    ...
    if (option.gdbp) {
      // 矢量图层
      queryService = new Zondy.Service.QueryLayerFeature(queryParam, {
        domain
      })
    } else {
      // 矢量文档
      ...
      queryService = new Zondy.Service.QueryDocFeature(
        queryParam,
        docName,
        layerIdxs,
        {
  ...
  public igsOnemapQueryFeature(option: FeatureQueryParam) {
    ...
    const structs = new Zondy.Service.QueryFeatureStruct({
      IncludeAttribute:
        option.IncludeAttribute !== undefined ? option.IncludeAttribute : true,
      IncludeGeometry:
        option.IncludeGeometry !== undefined ? option.IncludeGeometry : true,
  ...
  public getVectorByIndex(
    layerIdxs: string,
    vectorInfo: MapInfoCatalogLayer[],
    vectorArray: MapInfoCatalogLayer[] = [],
    layerIndex?: string
  ) {
    // 图层索引号数组
    if (!layerIdxs || layerIdxs === '' || !vectorInfo) {
      return []
    }
    if (!vectorArray) {
      vectorArray = []
    ...
    for (let i = 0; i < vectorInfo.length; i += 1) {
      ...
      for (let m = 0; m < layerIdxsArr.length; m += 1) {
        if (layerIdxsArr[m] === tip) {
          vectorArray.push(vectorInfo[i])
        }
      }
      if (vectorInfo[i].Type === 'Group') {
        if (vectorInfo[i].GroupMapLayerInfo) {
          for (let m = 0; m < layerIdxsArr.length; m += 1) {
            if (
              layerIdxsArr[m].includes('-') &&
              layerIdxsArr[m].split('-')[0] === tip
            ) {
              const { GroupMapLayerInfo } = vectorInfo[i]
              layerIndex = tip
              if (GroupMapLayerInfo) {
                vectorArray = this.getVectorByIndex(
                  layerIdxs,
                  GroupMapLayerInfo,
                  vectorArray,
                  layerIndex
      ...
      if (AttValue && tags) {
        for (let a = 0; a < AttValue.length; a += 1) {
          ...
          if (
            ...
            properties[tags[a]] = Number(val)
          } else {
            properties[tags[a]] = val
          }
        }
      }
  ...
  GeoJSONFeaturesToMapGIS(res: FeatureGeoJSON) {
    ...
    const featureSet = new Zondy.Object.FeatureSet(result)
    return featureSet
  }
