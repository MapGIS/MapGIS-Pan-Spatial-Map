...
export default class Mp3dDrawPro extends Mixins(DrawMixin, MapMixin) {
  // 绘制组件
  drawer = null
  ...
  // 绘制工具已经准备好
  private onAdded(drawer) {
    this.drawer = drawer
  }

  // 绘制结果拿到后
  private onDrawFinish(cartesian3, lnglat) {
    // 规范化结果，然后发送完成事件
    let geometry
    let coordinates
    let shape
