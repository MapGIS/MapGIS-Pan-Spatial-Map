...
define(['exports', './Transforms-49d48c2d', './Cartesian2-a7398fe3', './Check-f996273c', './when-ae2e0b60', './EllipsoidTangentPlane-a84086cc', './Math-ea295399', './Plane-3991c7d3'], (function (exports, Transforms, Cartesian2, Check, when, EllipsoidTangentPlane, Math$1, Plane) { 'use strict';
    ...
    function fromPlaneExtents(planeOrigin, planeXAxis, planeYAxis, planeZAxis, minimumX, maximumX, minimumY, maximumY, minimumZ, maximumZ, result) {
        //>>includeStart('debug', pragmas.debug);
        if (!when.defined(minimumX) || !when.defined(maximumX) || !when.defined(minimumY) || !when.defined(maximumY) || !when.defined(minimumZ) || !when.defined(maximumZ)) {
    ...
    OrientedBoundingBox.fromRectangle = function (rectangle, minimumHeight, maximumHeight, ellipsoid, result) {
        ...
        if (when.defined(ellipsoid) && !Math$1.CesiumMath.equalsEpsilon(ellipsoid.radii.x, ellipsoid.radii.y, Math$1.CesiumMath.EPSILON15)) {
        ...
        if (rectangle.width <= Math$1.CesiumMath.PI) {
            ...
            var perimeterCartographicNC = Cartesian2.Cartographic.fromRadians(lonCenter, rectangle.north, maximumHeight, scratchPerimeterCartographicNC);
            var perimeterCartographicNW = Cartesian2.Cartographic.fromRadians(rectangle.west, rectangle.north, maximumHeight, scratchPerimeterCartographicNW);
            var perimeterCartographicCW = Cartesian2.Cartographic.fromRadians(rectangle.west, latCenter, maximumHeight, scratchPerimeterCartographicCW);
            var perimeterCartographicSW = Cartesian2.Cartographic.fromRadians(rectangle.west, rectangle.south, maximumHeight, scratchPerimeterCartographicSW);
            var perimeterCartographicSC = Cartesian2.Cartographic.fromRadians(lonCenter, rectangle.south, maximumHeight, scratchPerimeterCartographicSC);
        ...
        var planeOrigin = Cartesian2.Cartesian3.fromRadians(centerLongitude, latitudeNearestToEquator, maximumHeight, ellipsoid, scratchPlaneOrigin);
        planeOrigin.z = 0.0; // center the plane on the equator to simpify plane normal calculation
        var isPole = Math.abs(planeOrigin.x) < Math$1.CesiumMath.EPSILON10 && Math.abs(planeOrigin.y) < Math$1.CesiumMath.EPSILON10;
        var planeNormal = !isPole ? Cartesian2.Cartesian3.normalize(planeOrigin, scratchPlaneNormal) : Cartesian2.Cartesian3.UNIT_X;
        ...
        var horizonCartesian = Cartesian2.Cartesian3.fromRadians(centerLongitude + Math$1.CesiumMath.PI_OVER_TWO, latitudeNearestToEquator, maximumHeight, ellipsoid, scratchHorizonCartesian);
        maxX = Cartesian2.Cartesian3.dot(Plane.Plane.projectPointOntoPlane(plane, horizonCartesian, scratchHorizonProjected), planeXAxis);
        minX = -maxX; // symmetrical

        // Get the min and max Y, using the height that will give the largest extent
        maxY = Cartesian2.Cartesian3.fromRadians(0.0, rectangle.north, fullyBelowEquator ? minimumHeight : maximumHeight, ellipsoid, scratchMaxY).z;
        minY = Cartesian2.Cartesian3.fromRadians(0.0, rectangle.south, fullyAboveEquator ? minimumHeight : maximumHeight, ellipsoid, scratchMinY).z;

        var farZ = Cartesian2.Cartesian3.fromRadians(rectangle.east, latitudeNearestToEquator, maximumHeight, ellipsoid, scratchZ);
    ...
    OrientedBoundingBox.intersectPlane = function (box, plane) {
        ...
        var radEffective =
            Math.abs(normalX * halfAxes[Transforms.Matrix3.COLUMN0ROW0] + normalY * halfAxes[Transforms.Matrix3.COLUMN0ROW1] + normalZ * halfAxes[Transforms.Matrix3.COLUMN0ROW2]) +
    ...
    var scratchValidAxis2 = new Cartesian2.Cartesian3();
    var scratchValidAxis3 = new Cartesian2.Cartesian3();
    ...
    OrientedBoundingBox.distanceSquaredTo = function (box, cartesian) {
        ...
        if (uHalf > 0) {
            Cartesian2.Cartesian3.divideByScalar(u, uHalf, u);
        } else {
            uValid = false;
        }

        if (vHalf > 0) {
            Cartesian2.Cartesian3.divideByScalar(v, vHalf, v);
        } else {
            vValid = false;
        }

        if (wHalf > 0) {
            Cartesian2.Cartesian3.divideByScalar(w, wHalf, w);
        } else {
            wValid = false;
        }
        ...
        if (numberOfDegenerateAxes === 1) {
            ...
            validAxis3 = Cartesian2.Cartesian3.cross(validAxis1, validAxis2, scratchValidAxis3);
            ...
            var crossVector = Cartesian2.Cartesian3.UNIT_Y;
            if (crossVector.equalsEpsilon(validAxis1, Math$1.CesiumMath.EPSILON3)) {
                crossVector = Cartesian2.Cartesian3.UNIT_X;
            }

            validAxis2 = Cartesian2.Cartesian3.cross(validAxis1, crossVector, scratchValidAxis2);
            Cartesian2.Cartesian3.normalize(validAxis2, validAxis2);
            validAxis3 = Cartesian2.Cartesian3.cross(validAxis1, validAxis2, scratchValidAxis3);
            Cartesian2.Cartesian3.normalize(validAxis3, validAxis3);
            ...
            u = Cartesian2.Cartesian3.UNIT_X;
            v = Cartesian2.Cartesian3.UNIT_Y;
            w = Cartesian2.Cartesian3.UNIT_Z;
        }
    ...
    OrientedBoundingBox.equals = function (left, right) {
        return left === right || (when.defined(left) && when.defined(right) && Cartesian2.Cartesian3.equals(left.center, right.center) && Transforms.Matrix3.equals(left.halfAxes, right.halfAxes));
    };
...
//# sourceMappingURL=OrientedBoundingBox-504b2ab6.js.map
