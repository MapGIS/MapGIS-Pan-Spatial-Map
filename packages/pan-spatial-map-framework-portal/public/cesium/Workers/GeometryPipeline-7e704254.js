...
define(['exports', './AttributeCompression-75c625a9', './Cartesian2-cdd21d41', './Check-c47ad5ed', './when-695b7bde', './Math-2649012a', './Transforms-b822f480', './ComponentDatatype-66ac9db6', './EncodedCartesian3-a0fc8560', './GeometryAttribute-13ef0da1', './IndexDatatype-a938a6f6', './IntersectionTests-492833eb', './Plane-ab4468be'], (function (exports, AttributeCompression, Cartesian2, Check, when, Math$1, Transforms, ComponentDatatype, EncodedCartesian3, GeometryAttribute, IndexDatatype, IntersectionTests, Plane) { 'use strict';
    ...
    function barycentricCoordinates(point, p0, p1, p2, result) {
        ...
        if (!when.defined(p0.z)) {
            ...
            if (Cartesian2.Cartesian3.equalsEpsilon(point, p0, Math$1.CesiumMath.EPSILON14)) {
                return Cartesian2.Cartesian3.clone(Cartesian2.Cartesian3.UNIT_X, result);
            }
            if (Cartesian2.Cartesian3.equalsEpsilon(point, p1, Math$1.CesiumMath.EPSILON14)) {
                return Cartesian2.Cartesian3.clone(Cartesian2.Cartesian3.UNIT_Y, result);
            }
            if (Cartesian2.Cartesian3.equalsEpsilon(point, p2, Math$1.CesiumMath.EPSILON14)) {
                return Cartesian2.Cartesian3.clone(Cartesian2.Cartesian3.UNIT_Z, result);
            }
    ...
    GeometryPipeline.reorderForPreVertexCache = function (geometry) {
        ...
        if (when.defined(indices)) {
            ...
            for (var property in attributes) {
                if (attributes.hasOwnProperty(property) && when.defined(attributes[property]) && when.defined(attributes[property].values)) {
    ...
    function copyAttributesDescriptions(attributes) {
        ...
        for (var attribute in attributes) {
            if (attributes.hasOwnProperty(attribute) && when.defined(attributes[attribute]) && when.defined(attributes[attribute].values)) {
    ...
    function copyVertex(destinationAttributes, sourceAttributes, index) {
        for (var attribute in sourceAttributes) {
            if (sourceAttributes.hasOwnProperty(attribute) && when.defined(sourceAttributes[attribute]) && when.defined(sourceAttributes[attribute].values)) {
    ...
    GeometryPipeline.fitToUnsignedShortIndices = function (geometry) {
        ...
        if (when.defined(geometry.indices) && numberOfVertices >= Math$1.CesiumMath.SIXTY_FOUR_KILOBYTES) {
            var oldToNewIndex = [];
            var newIndices = [];
            var currentIndex = 0;
            ...
            for (var j = 0; j < numberOfIndices; j += indicesPerPrimitive) {
                ...
                if (currentIndex + indicesPerPrimitive >= Math$1.CesiumMath.SIXTY_FOUR_KILOBYTES) {
                    geometries.push(
    ...
    function transformVector(matrix, attribute) {
        if (when.defined(attribute)) {
            ...
            for (var i = 0; i < length; i += 3) {
                ...
                scratchCartesian3 = Cartesian2.Cartesian3.normalize(scratchCartesian3, scratchCartesian3);
                Cartesian2.Cartesian3.pack(scratchCartesian3, values, i);
            }
        }
    }
    ...
    GeometryPipeline.transformToWorldCoordinates = function (instance) {
        ...
        if (when.defined(attributes.normal) || when.defined(attributes.tangent) || when.defined(attributes.bitangent)) {
    ...
    function findAttributesInAllGeometries(instances, propertyName) {
        ...
        for (name in attributes0) {
            if (attributes0.hasOwnProperty(name) && when.defined(attributes0[name]) && when.defined(attributes0[name].values)) {
    ...
    GeometryPipeline.computeNormal = function (geometry) {
        ...
        for (i = 0; i < numVertices; i++) {
            ...
            if (vertexNormalData.count > 0) {
                ...
                if (Cartesian2.Cartesian3.equalsEpsilon(Cartesian2.Cartesian3.ZERO, normal, Math$1.CesiumMath.EPSILON10)) {
    ...
    GeometryPipeline.compressVertices = function (geometry) {
        ...
        for (i = 0; i < numVertices; ++i) {
            if (hasSt) {
                ...
                compressedAttributes[normalIndex++] = AttributeCompression.AttributeCompression.compressTextureCoordinates(scratchCartesian2);
            }
            ...
            if (hasNormal && when.defined(tangents) && when.defined(bitangents)) {
                ...
                AttributeCompression.AttributeCompression.octPack(toEncode1, toEncode2, toEncode3, scratchCartesian2);
    ...
    function splitLongitudeTriangles(instance) {
        ...
        for (i = 0; i < len; i += 3) {
            ...
            console.log("defined(result) && result.positions.length > 3",when.defined(result) && result.positions.length > 3);
    ...
    function computeLineAttributes(i0, i1, point, positions, insertIndex, currentAttributes, applyOffset) {
        ...
        var p0 = Cartesian2.Cartesian3.fromArray(positions, i0 * 3, p0Scratch);
        if (Cartesian2.Cartesian3.equalsEpsilon(p0, point, Math$1.CesiumMath.EPSILON10)) {
    ...
    function splitLongitudeLines(instance) {
        ...
        for (i = 0; i < length; i += 2) {
            ...
            if (when.defined(intersection)) {
                ...
                var offset = Cartesian2.Cartesian3.multiplyByScalar(Cartesian2.Cartesian3.UNIT_Y, 5.0 * Math$1.CesiumMath.EPSILON9, offsetScratch);
                if (p0.y < 0.0) {
    ...
    var offsetScalar = 5.0 * Math$1.CesiumMath.EPSILON9;
    var coplanarOffset = Math$1.CesiumMath.EPSILON6;
        ...
        for (i = 0; i < length; i += 4) {
            ...
            if (when.defined(intersection)) {
                ...
                if (when.defined(colors)) {
                    ...
                    var r = Math$1.CesiumMath.lerp(c0.x, c2.x, t);
                    var g = Math$1.CesiumMath.lerp(c0.y, c2.y, t);
                    var b = Math$1.CesiumMath.lerp(c0.z, c2.z, t);
                    var a = Math$1.CesiumMath.lerp(c0.w, c2.w, t);
                ...
                if (when.defined(texCoords)) {
                    ...
                    var sx = Math$1.CesiumMath.lerp(s0.x, s3.x, t);
...
//# sourceMappingURL=GeometryPipeline-7e704254.js.map
