...
define(['exports', './GeometryOffsetAttribute-ee7c1230', './Transforms-428723c7', './Cartesian2-a7398fe3', './Check-f996273c', './ComponentDatatype-9859adcf', './when-ae2e0b60', './EllipseGeometryLibrary-85dcca0e', './GeometryAttribute-24498485', './GeometryAttributes-5ce4955a', './GeometryInstance-d26b123e', './GeometryPipeline-37decdab', './IndexDatatype-386a3275', './Math-ea295399', './VertexFormat-69b61abe'], (function (exports, GeometryOffsetAttribute, Transforms, Cartesian2, Check, ComponentDatatype, when, EllipseGeometryLibrary, GeometryAttribute, GeometryAttributes, GeometryInstance, GeometryPipeline, IndexDatatype, Math$1, VertexFormat) { 'use strict';
    ...
    var projectedCenterScratch = new Cartesian2.Cartesian3();
    ...
    function computeTopBottomAttributes(positions, options, extrude) {
        ...
        var projection = new Transforms.GeographicProjection(ellipsoid);
        ...
        if (stRotation !== 0) {
            ...
            tangentMatrix = Transforms.Matrix3.fromQuaternion(rotation, tangentMatrix);
        } else {
            textureMatrix = Transforms.Matrix3.clone(Transforms.Matrix3.IDENTITY, textureMatrix);
            tangentMatrix = Transforms.Matrix3.clone(Transforms.Matrix3.IDENTITY, tangentMatrix);
        }
        ...
        for (var i = 0; i < length; i += 3) {
            ...
            if (vertexFormat.st) {
                var rotatedPoint = Transforms.Matrix3.multiplyByVector(textureMatrix, position, scratchCartesian2);
                ...
                Cartesian2.Cartesian3.subtract(projectedPoint, projectedCenter, projectedPoint);
            ...
            if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent || shadowVolume) {
                ...
                if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
                    if (vertexFormat.tangent || vertexFormat.bitangent) {
                        tangent = Cartesian2.Cartesian3.normalize(Cartesian2.Cartesian3.cross(Cartesian2.Cartesian3.UNIT_Z, normal, tangent), tangent);
                        Transforms.Matrix3.multiplyByVector(tangentMatrix, tangent, tangent);
                    }
    ...
    function computeEllipse(options) {
        var center = options.center;
        boundingSphereCenter = Cartesian2.Cartesian3.multiplyByScalar(options.ellipsoid.geodeticSurfaceNormal(center, boundingSphereCenter), options.height, boundingSphereCenter);
        boundingSphereCenter = Cartesian2.Cartesian3.add(center, boundingSphereCenter, boundingSphereCenter);
    ...
    function computeWallAttributes(positions, options) {
        ...
        ellipsoid.geodeticSurfaceNormal(geodeticNormal, geodeticNormal);
        var rotation = Transforms.Quaternion.fromAxisAngle(geodeticNormal, stRotation, quaternionScratch);
        var textureMatrix = Transforms.Matrix3.fromQuaternion(rotation, textureMatrixScratch);
        ...
        for (var i = 0; i < length; i += 3) {
            ...
            extrudedPosition = Cartesian2.Cartesian3.clone(position, scratchCartesian2);
            normal = ellipsoid.geodeticSurfaceNormal(position, normal);
            ...
            var scaledNormal = Cartesian2.Cartesian3.multiplyByScalar(normal, height, scratchCartesian4);
            position = Cartesian2.Cartesian3.add(position, scaledNormal, position);
            scaledNormal = Cartesian2.Cartesian3.multiplyByScalar(normal, extrudedHeight, scaledNormal);
            extrudedPosition = Cartesian2.Cartesian3.add(extrudedPosition, scaledNormal, extrudedPosition);
            ...
            if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
                bitangent = Cartesian2.Cartesian3.clone(normal, bitangent);
                var next = Cartesian2.Cartesian3.fromArray(positions, (i + 3) % length, scratchCartesian4);
                Cartesian2.Cartesian3.subtract(next, position, next);
                var bottom = Cartesian2.Cartesian3.subtract(extrudedPosition, position, scratchCartesian3);

                normal = Cartesian2.Cartesian3.normalize(Cartesian2.Cartesian3.cross(bottom, next, normal), normal);
    ...
    function computeExtrudedEllipse(options) {
        ...
        var ellipsoid = options.ellipsoid;
        var semiMajorAxis = options.semiMajorAxis;
        var scaledNormal = Cartesian2.Cartesian3.multiplyByScalar(ellipsoid.geodeticSurfaceNormal(center, scratchCartesian1), options.height, scratchCartesian1);
        topBoundingSphere.center = Cartesian2.Cartesian3.add(center, scaledNormal, topBoundingSphere.center);
        topBoundingSphere.radius = semiMajorAxis;

        scaledNormal = Cartesian2.Cartesian3.multiplyByScalar(ellipsoid.geodeticSurfaceNormal(center, scaledNormal), options.extrudedHeight, scaledNormal);
        bottomBoundingSphere.center = Cartesian2.Cartesian3.add(center, scaledNormal, bottomBoundingSphere.center);
    ...
    function computeRectangle(center, semiMajorAxis, semiMinorAxis, rotation, granularity, ellipsoid, result) {
        ...
        for (var i = 0; i < positionsCount; ++i) {
            positions[i] = Cartesian2.Cartesian3.fromArray(positionsFlat, i * 3);
        }
        ...
        if (rectangle.width > Math$1.CesiumMath.PI) {
            rectangle.north = rectangle.north > 0.0 ? Math$1.CesiumMath.PI_OVER_TWO - Math$1.CesiumMath.EPSILON7 : rectangle.north;
            rectangle.south = rectangle.south < 0.0 ? Math$1.CesiumMath.EPSILON7 - Math$1.CesiumMath.PI_OVER_TWO : rectangle.south;
    ...
    function EllipseGeometry(options) {
        ...
        var semiMajorAxis = options.semiMajorAxis;
        var semiMinorAxis = options.semiMinorAxis;
        var granularity = when.defaultValue(options.granularity, Math$1.CesiumMath.RADIANS_PER_DEGREE);
     ...
     * @type {Number}
     */
    EllipseGeometry.packedLength = Cartesian2.Cartesian3.packedLength + Cartesian2.Ellipsoid.packedLength + VertexFormat.VertexFormat.packedLength + 9;
...
//# sourceMappingURL=EllipseGeometry-a89724f5.js.map
